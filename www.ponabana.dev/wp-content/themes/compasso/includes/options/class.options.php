...
class G7_Options {
	...
	function display_fields($args = array()) {
		...
		switch ($args['type']) {
			...
			case 'texts':
				$input = '<div class="g7-item" id="%s_%s"><input type="text" name="%s" value="%s" class="regular-text" %s> <a href="#" class="g7-item-delete"><img src="%s/images/cross.png"></a></div>';
				$input2 = '';
				...
				printf(
					'<div class="g7-item-container">%s</div><div><a class="g7-item-add" href="#" data-id="%s" data-title="%s">Add %s</a></div>',
					$input2,
					$args['name'],
					$args['title'],
					$args['title']
		...
		<div class="wrap<?php echo $this->use_tabs ? ' use_tabs' : ''; ?>">
			...
			<form method="post" action="options.php" enctype="multipart/form-data" id="g7-options">
				...
				<p class="submit">
					...
					<input type="submit" class="button-secondary" name="reset" value="<?php _e('Reset', 'g7theme'); ?>" onclick="return confirm('<?php _e('Data will be reset to default. Are you sure?', 'g7theme'); ?>')" />
					<?php if ($this->use_tabs) : ?>
